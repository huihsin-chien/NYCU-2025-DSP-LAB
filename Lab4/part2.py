...
print(f"對應質心時間差 = {best_centroid:.3f} (100 ps)")
...
plt.axvline(best_centroid, color='red', linestyle='--', label=f'Estimated peak: {best_centroid:.1f} (100 ps)\n({distance_m:.2f} m)')
plt.title('Time-Of-Flight Histogram(window+centroid)')
plt.xlabel('Time difference (100 ps)')
plt.ylabel('Count')
plt.legend()
plt.grid(True)
plt.show()
