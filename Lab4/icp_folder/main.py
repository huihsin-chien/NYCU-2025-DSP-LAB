...
def main():
    ...
    print("Saved rotated-view depth PNG (gray):", OUT_ROTATED_PNG)
       ...
       # ---- (C) 輪廓圖：使用深度梯度放大 (depth gradient boost) ----
    # 先稍微平滑，減少 noise
    depth_blur = cv2.GaussianBlur(depth_gray, (5, 5), 0)

    depth_f = depth_blur.astype(np.float32)
    ...
    cv2.imwrite(OUT_EDGES_PNG, grad_mag)
    print("Saved depth-gradient edge PNG:", OUT_EDGES_PNG)
